...
resource "null_resource" "ansible-exec-non-ha" {
  ...
  provisioner "local-exec" {
    command = "ssh -o 'StrictHostKeyChecking no' -i ${var.ID_RSA_FILE_PATH} root@${var.BASTION_FLOATING_IP} 'export ANSIBLE_CONFIG=/tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}/ansible.cfg; nohup ansible-playbook --private-key /tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}/id_rsa -i /tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}/inventory.yml /tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}/non_ha_hana_backup.yml > /tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}.log 2>&1 &'"
  }
}
...
resource "null_resource" "ansible-exec-ha" {
  ...
  provisioner "local-exec" {
    command = "ssh -o 'StrictHostKeyChecking no' -i ${var.ID_RSA_FILE_PATH} root@${var.BASTION_FLOATING_IP} 'export ANSIBLE_CONFIG=/tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}/ansible.cfg; nohup ansible-playbook --private-key /tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}/id_rsa -i /tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}/inventory.yml /tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}/ha_hana_backup.yml > /tmp/ansible.${local.SAP_DEPLOYMENT}-${var.IP}.log 2>&1 &'"
  }
}
...
resource "null_resource" "ansible-delete-sensitive-data" {
    ...
    depends_on	= [ null_resource.ansible-exec-non-ha, null_resource.ansible-exec-ha, null_resource.ansible-logs4 ]
